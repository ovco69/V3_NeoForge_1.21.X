...
public class ChiselItem extends Item {
        ...
        if (CHISEL_MAP.containsKey(clickedBlock)) {
            if (!level.isClientSide()) {
                ...
                ((ServerLevel) level).sendParticles(
                        ModParticles.BISMUTH_PARTICLES.get(),
                        context.getClickedPos().getX() + .5,
                        context.getClickedPos().getY() + 1,
                        context.getClickedPos().getZ() + .5,
                        5,
                        0,
                        0,
                        0,
    ...
    public void appendHoverText(ItemStack stack, TooltipContext context, List<Component> tooltipComponents, TooltipFlag tooltipFlag) {
        if (Screen.hasShiftDown()) {
            tooltipComponents.add(Component.translatable("tooltip." + OmniGear.MOD_ID + ".chisel.shift_down"));
        }
        else {
            tooltipComponents.add(Component.translatable("tooltip." + OmniGear.MOD_ID + ".chisel"));
        }
