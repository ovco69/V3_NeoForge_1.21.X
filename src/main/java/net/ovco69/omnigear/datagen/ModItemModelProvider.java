...
public class ModItemModelProvider extends ItemModelProvider {
    ...
    public ModItemModelProvider(PackOutput output, ExistingFileHelper existingFileHelper) {
        super(output, OmniGear.MOD_ID, existingFileHelper);
    }
    ...
    protected void registerModels() {
        ...
        basicItem(ModItems.ARTIST_BLOCK_MUSIC_DISC.get());
    ...
    private ItemModelBuilder saplingItem(DeferredBlock<Block> item) {
        return withExistingParent(item.getId().getPath(),
                ResourceLocation.parse("item/generated")).texture(
                        "layer0", ResourceLocation.fromNamespaceAndPath(OmniGear.MOD_ID, "block/" + item.getId().getPath())
        );
    }

    private void trimmedArmorItem(DeferredItem<ArmorItem> itemDeferredItem) {
        final String MOD_ID = OmniGear.MOD_ID;
    ...
    public void buttonItem(DeferredBlock<?> block, DeferredBlock<Block> baseBlock) {
        this.withExistingParent(block.getId().getPath(), mcLoc("block/button_inventory"))
                .texture(
                        "texture",
                        ResourceLocation.fromNamespaceAndPath(OmniGear.MOD_ID, "block/" + baseBlock.getId().getPath()));
    }
    ...
    private ItemModelBuilder handheldItem(DeferredItem<?> item) {
        return withExistingParent(item.getId().getPath(),
                ...
                ResourceLocation.fromNamespaceAndPath(OmniGear.MOD_ID, "item/" + item.getId().getPath()));
    }
}
